...
// GalleryModal — full-screen selected item view with draggable dock
    ...
    <>
      <motion.div
        ...
        className="fixed inset-0 w-full min-h-screen sm:h-[90vh] md:h-[600px]
                   backdrop-blur-lg rounded-none sm:rounded-lg md:rounded-xl overflow-hidden z-10">
            ...
            <AnimatePresence mode="wait">
              <motion.div key={selectedItem.id}
                className="relative w-full aspect-[16/9] max-w-[95%] sm:max-w-[85%] md:max-w-3xl
                           h-auto max-h-[70vh] rounded-xl overflow-hidden shadow-md"
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 md:p-5
                  ...
                  <p className="text-white/75 text-xs sm:text-sm mt-1">{selectedItem.desc}</p>
                  {/* C-Cure CTA — shown if item has a link */}
                  {selectedItem.link && (
                    <Link to={selectedItem.link}
                      onClick={e => e.stopPropagation()}
                      className="inline-flex items-center gap-1.5 mt-3 px-4 py-2 rounded-full text-xs font-medium
                                 bg-white text-[#A41162] hover:-translate-y-0.5 transition-all duration-200 shadow-sm">
                      Bekijk alle behandelingen <ArrowRight size={12} />
                    </Link>
                  )}
                </div>
              </motion.div>
      ...
      <motion.div drag dragMomentum={false} dragElastic={0.1} initial={false}
        ...
        onDragEnd={(_, info) => setDockPosition(prev => ({
          x: prev.x + info.offset.x, y: prev.y + info.offset.y
        }))}
        className="fixed z-50 left-1/2 bottom-4 -translate-x-1/2 touch-none">
        <motion.div className="relative rounded-xl bg-white/20 backdrop-blur-xl
                               border border-white/30 shadow-lg cursor-grab active:cursor-grabbing">
          <div className="flex items-center -space-x-2 px-3 py-2">
            {mediaItems.map((item, index) => (
              <motion.div key={item.id}
                onClick={e => { e.stopPropagation(); setSelectedItem(item) }}
                style={{ zIndex: selectedItem.id === item.id ? 30 : mediaItems.length - index }}
                className={`relative group w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 flex-shrink-0
      ...
      {title && (
        <div className="mb-8 text-center">
          <motion.h2
            ...
            style={{ fontFamily: 'Cormorant Garamond, Georgia, serif', color: '#2C2028' }}
            initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            {title}
          </motion.h2>
          {description && (
            <motion.p className="mt-2 text-sm sm:text-base text-[#7A6270]"
              ...
              <motion.div key={item.id} layoutId={`media-${item.id}`}
                ...
                <motion.div className="absolute inset-0 flex flex-col justify-end"
                  ...
                  <div className="relative p-3 sm:p-4">
                    <h3 className="text-white text-xs sm:text-sm md:text-base font-semibold line-clamp-1">
                      {item.title}
                    </h3>
                ...
                {/* Always-visible bottom strip with category accent */}
                {item.accentColor && (
